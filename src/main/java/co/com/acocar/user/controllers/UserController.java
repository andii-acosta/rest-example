...
public class UserController {
	...
	@GetMapping(value = "/")
	public ResponseEntity<?> getUSerList(@RequestHeader(value = "token", required = true) String token)
			throws Exception {
		List<User> list = null;
	...
	@ApiResponse(responseCode = "400", description = "Error credentials")
	@PutMapping(value = "/")
	public ResponseEntity<?> updateUser(@RequestHeader(value = "token", required = true) String token,
			@RequestBody User u) {
		if (validarToken(token)) {
